<script>
    ...
    export default {
      ...
      mounted() {
        axios.get(API_URL)
          .then((response) => {
            this.info = response.data.personas;
 ...
 */
    ...
    <template  >
        ...
        <div class="flex justify-center">
          ...
          <div class=" mr-3 mb-3">
            <input type="text" v-model="buscar" class="form-control rounded-lg px-3 py-1" placeholder="Encuentra algo"/>
          </div>
        ...
        <div v-if="buscar.length !== 0" v-for="p in personajesEncontrados" class= "border-solid border-2 rounded-lg border-current mb-6 ml-6 mr-6">
          <div class="object-center justify-center flex card-text">
            ...
            <div v-for="m in info" class= " border-solid border-2 rounded-lg border-current mb-6 ml-6 mr-6">
                  <!-- {{m}} -->
              ...
              <div class="object-center justify-center flex card-text">
                <div class="center card-body">
                  ...
                  <h2 class="font-bold text-3xl text-center mt-4">
                      ...
                      <RouterLink to="/vistaPersona"><button class="underline text-blue-600" :ejemplo="1">{{m.nombre}}</button>
                        </RouterLink>
                  </h2>

                  <div class=" ml-3 mb-5 mt-3 card-text text-xl">
                            
                            <p><strong>Residencia:</strong>  {{m.residencia}}  </p>
                            <p><strong>Trabajo:</strong>  {{m.trabajo}}  </p> 
                            <p><strong>Edad:</strong>  {{m.edad}}  </p>
                            <p><strong>Biografía:</strong>  {{m.bio}}  </p>
                            <p><strong>Cita:</strong>  {{m.cita}}  </p>
                            <p><strong>Autor de la cita:</strong>  {{m.citaAutor}}  </p>
                            <p><strong>Estado civil:</strong>  {{m.estadoCivil}}  </p>
                  </div>
                </div>
